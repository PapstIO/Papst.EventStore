...
namespace Papst.EventStore.CodeGeneration
{
  ...
  public partial class EventRegistrationIncrementalCodeGenerator : IIncrementalGenerator
  {
    ...
    public void Initialize(IncrementalGeneratorInitializationContext context)
    {
      context.RegisterSourceOutput(context.CompilationProvider, (productionContext, compilation) =>
      {
        try
        {
          ...
          if (entryPoint != null && !IsGeneratedEntryPoint(entryPoint, productionContext.CancellationToken))
          {
    ...
    /// <summary>
    /// True when the entry point exists only in generated code, in which case its namespace must not
    /// drive the namespace of the generated registration. Test frameworks synthesise an entry point in
    /// a namespace of their own — xunit.v3 uses a randomised <c>Xunit.AutoGenerated._xxxxx</c> — which
    /// would otherwise make the generated members unreachable and unstable between builds.
    /// </summary>
    private static bool IsGeneratedEntryPoint(ISymbol entryPoint, CancellationToken cancellationToken)
    {
      var references = entryPoint.DeclaringSyntaxReferences;
      ...
      {
        if (!IsGeneratedCode(reference.SyntaxTree, cancellationToken))
        {
          return false;
        }
      }
    ...
    /// <summary>
    /// Applies the conventional generated-code heuristics: a generated file name, or a leading
    /// <c>&lt;auto-generated&gt;</c> banner comment.
    /// </summary>
    private static bool IsGeneratedCode(SyntaxTree tree, CancellationToken cancellationToken)
    {
      ...
      // An empty path means an in-memory tree of unknown origin, not a generated one; fall through
      // to the banner check rather than assuming either way.
