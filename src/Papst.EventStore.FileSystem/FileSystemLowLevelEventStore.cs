...
internal class FileSystemLowLevelEventStore : ILowLevelEventStore
{
  private readonly ILoggerFactory _loggerFactory;
  private readonly ILogger<FileSystemLowLevelEventStore> _logger;
  private readonly string _path;
  private readonly IEventTypeProvider _eventTypeProvider;

  public FileSystemLowLevelEventStore(ILogger<FileSystemLowLevelEventStore> logger, IOptions<FileSystemEventStoreOptions> options, ILoggerFactory loggerFactory, IEventTypeProvider eventTypeProvider)
  {
  ...
  public async Task<ILowLevelEventStream> CreateAsync(Guid streamId, string targetTypeName, CancellationToken cancellationToken = default) => await CreateAsync(
      streamId,
      targetTypeName,
      null,
      null,
  ...
  public async Task<ILowLevelEventStream> CreateAsync(
    Guid streamId,
    ...
    CancellationToken cancellationToken = default)
  {
    ...
    await File.WriteAllTextAsync(Path.Combine(streamPath, FileSystemEventStore.IndexFileName), JsonSerializer.Serialize(entity), cancellationToken).ConfigureAwait(false);

    ILowLevelEventStream stream = new FileSystemEventStream(_loggerFactory.CreateLogger<FileSystemEventStream>(), streamPath, entity, _eventTypeProvider);
    return stream;
  }

  public async Task<ILowLevelEventStream> GetAsync(Guid streamId, CancellationToken cancellationToken = default)
  {
    ...
    string indexFile = Path.Combine(streamPath, FileSystemEventStore.IndexFileName);
